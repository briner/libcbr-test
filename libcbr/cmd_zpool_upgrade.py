...
class CmdZpoolUpgrade(mexecutor.ShellCmdWrapped):
    def __new__(self):
        return str.__new__(self, "zpool upgrade")
    def __init__(self):
        super(CmdZpoolUpgrade, self).__init__("zpool upgrade")
    with_construct=True
...
class PoolnameVersion(object):
    ...
    def __repr__(self):
        return "pool(%s) version(%s)" % (self.name, self.version)
